      ...
      Form {
        Section {
          ...
          VStack(alignment: .leading, spacing: 8) {
            Text(
              "Enter your RetroDiffusion API key. If set, this will override the key from Config.plist."
            )

            Link(
              "Get your API key", destination: URL(string: Constants.URLs.retrodiffusionWebsite)!
            )
            .font(.footnote)
          }
          .padding(.top, 4)
